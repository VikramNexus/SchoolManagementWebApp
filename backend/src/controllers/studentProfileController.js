 ...
 * Export complete individual student profile & running balance ledger dossier (.xlsx) matching photo layout
 */
async function exportStudentProfileExcel(req, res) {
  ...
  try {
    const student = await db.queryOne(
      'SELECT id, admission_no, full_name FROM students WHERE id = ? AND status != "deleted"',
      [id]
    );
    ...
    if (!res.headersSent) {
      return res.status(500).json({ success: false, message: 'Failed to export student ledger Excel: ' + err.message });
    }
  }
}
